<?php

return [

    "title" => "Donnez",
    "scripture" => "« ...puisqu’il a lui-même dit: ‘Il y a plus de bonheur à donner qu’à recevoir.’ » - Actes 20.35",

    "introduction" => "Merci de votre intérêt à aider notre ministère. Comme nous sommes encouragés à donner librement, conformément au commandement de Jésus, tout soutien financier est apprécié pour faire avancer l’œuvre de Dieu à Montréal, au Québec, au Canada et dans le monde entier. Ces ressources servent à couvrir les dépenses liées à l’entretien de notre église et à soutenir la mission mondiale des campus. Il existe plusieurs façons de donner :",

    "inperson" => ["title" => "En personne", "instructions" => "Au moment de l'offrande, pendant l'adoration les dimanches."],

    "online" => ["title" => "En ligne", "instructions" => "Nous avons créé un compte Zeffy pour vous permettre de faire des dons en ligne lorsque vous scanner le code QR affiché ou en cliquant sur l'image ci-dessous pour accéder au site.",
    "note" => "Remarque : vous n’êtes pas obligé de payer des frais de service. Veuillez observer les instructions de la vidéo pour savoir comment faire."],

    "charity" => "Malheureusement, nous ne pouvons pas remettre de reçus fiscaux, car notre ministère n’est pas enregistré à titre d’organisme de bienfaisance auprès du gouvernement fédéral. Cette décision a été prise par l’ensemble des membres de notre communauté afin que nous puissions soutenir librement nos ministères sœurs partout dans le monde.",

];