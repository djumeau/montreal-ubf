    ...
    <div class="max-w-4xl mx-auto p-8 bg-white rounded-2xl border border-slate-100 shadow-xs">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-5 mb-6">
            ...
            <div class="flex bg-slate-100 p-1 rounded-xl mt-4 sm:mt-0 self-start">
                <button type="button" @click="currentTab = 'en'"
                    :class="currentTab === 'en' ? 'bg-white shadow-xs text-indigo-600 font-semibold' :
                        'text-slate-600 hover:text-slate-900'"
                    class="px-4 py-1.5 rounded-lg text-sm transition cursor-pointer">
                    EN
                </button>
                <button type="button" @click="currentTab = 'fr'"
                    :class="currentTab === 'fr' ? 'bg-white shadow-xs text-indigo-600 font-semibold' :
                        'text-slate-600 hover:text-slate-900'"
                    class="px-4 py-1.5 rounded-lg text-sm transition cursor-pointer">
                    FR
                </button>
        ...
        <form action="{{ route('bible-studies.store') }}" method="POST" class="space-y-6">
            ...
            <div
                class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50/50 p-5 rounded-xl border border-slate-100 mb-2">

                <!-- Insert Text Input Component for Book ID -->
            ...
            @foreach (['en' => 'English Content Panel', 'fr' => 'Panneau de contenu Français'] as $lang => $label)
                <div x-show="currentTab === '{{ $lang }}'" class="space-y-6"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">Lesson Title
                                ({{ strtoupper($lang) }})
                            </label>
                            <input type="text" name="title[{{ $lang }}]" value="{{ old("title.$lang") }}"
                                placeholder="{{ $lang === 'en' ? 'e.g., Jesus is God' : 'ex., Jésus est Dieu' }}"
                        ...
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">Bible Passage
                                ({{ strtoupper($lang) }})</label>
                            <input type="text" name="bible_passage[{{ $lang }}]"
                                value="{{ old("bible_passage.$lang") }}"
                                placeholder="{{ $lang === 'en' ? 'e.g., 1:1-4' : 'ex., 12.1-4' }}"
                        ...
                        <!-- Image Links -->
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">Image Links
                                ({{ strtoupper($lang) }}) <span class="text-xs text-slate-400 font-normal">(One URL per
                                    line)</span></label>
                        ...
                        <!-- Passage Links -->
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">Passage Links
                                ({{ strtoupper($lang) }}) <span class="text-xs text-slate-400 font-normal">(One URL per
                                    line)</span></label>
                        ...
                        <!-- Question Sheet Text -->
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">Question Sheet Text / URL
                                ({{ strtoupper($lang) }})</label>
                            <input type="text" name="question_sheet[{{ $lang }}]"
                                value="{{ old("question_sheet.$lang") }}"
                        ...
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-slate-700 mb-1.5"Lecture
                                ({{ strtoupper($lang) }})</label>
            ...
            @if ($errors->any())
                <div class="p-3 bg-red-50 border border-red-100 rounded-xl">
                    <p class="text-xs text-red-600 font-medium">Please review both English and French fields. Missing
                        translations detected.</p>
                </div>
            @endif
