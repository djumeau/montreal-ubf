@props([
    ...
    'label' => null,
    'accept' => null,
    'multiple' => false, // Allow several files; submitted as name[] and validated as name / name.*
...
@php
    ...
    // With several files, errors come back as name (none chosen) or name.0, name.1... (one bad file)
    $fieldError = $fieldErrors->first($name) ?: ($multiple ? $fieldErrors->first($name . '.*') : '');
@endphp
...
<div {{ $attributes->merge(['class' => 'mb-4']) }} x-data="{ fileName: '' }">
    ...
    <label
        class="flex items-center gap-3 w-full border rounded py-2 px-3 text-sm cursor-pointer hover:bg-slate-700/50 focus-within:ring-2 focus-within:ring-sky-500 {{ $fieldError ? 'border-red-500' : 'border-slate-300' }}">

        <input id="{{ $id }}" name="{{ $multiple ? $name . '[]' : $name }}" type="file" @if ($accept) accept="{{ $accept }}" @endif
            @if ($multiple) multiple @endif
            class="sr-only" @change="fileName = Array.from($event.target.files).map(file => file.name).join(', ')" />
        ...
        <!-- Newly chosen file(s), else the current file, else "No file chosen" -->
